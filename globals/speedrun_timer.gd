...
func start_speedrun() -> void:
	...
	level_times.fill(0.0)
	start_level_timer()


func end_speedrun_early() -> void:
...
func on_level_exit(level_int: int, elapsed_time: int) -> void:
	...
	if level_int == final_level:
		get_tree().change_scene_to_file("res://speedrun_finished/speedrun_finished.tscn")
	
	if is_speedrunning and level_int == final_level:
		...
		for time: int in level_times:
			if time != 0:
				...
				return
		get_tree().change_scene_to_file("res://speedrun_finished/speedrun_finished.tscn") # Add a speedrun finished screen.
